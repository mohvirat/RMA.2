...
def match_responsibilities_to_jd(resume_text, jd_text):
    prompt = f"""
    Compare the candidate's resume responsibilities with the job description below.
    Respond with:
    - Matching responsibilities (bullet points)
    - Gaps or missing responsibilities
    - Overall alignment score out of 100
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a technical recruiter evaluating resume alignment with job descriptions."},
                {"role": "user", "content": prompt}
            ]
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        return f"Responsibility match failed: {e}"

def analyze_resume_profile(text):
    prompt = f"""
    You are a technical recruiter. Read and analyze the resume content below. Extract the candidate's role, level of seniority, technologies, key responsibilities, and domain expertise.

    Resume:
    {text[:4000]}

    Provide a structured understanding for further matching.
    """
    try:
        client = openai.OpenAI(api_key=api_key)
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are a senior recruiter evaluating a candidate's profile."},
                {"role": "user", "content": prompt}
            ]
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        return f"Resume understanding failed: {e}"
    ...
    if st.session_state.authenticated:
        ...
        if jd_text_input and resume_files and run_eval:
            ...
            with st.spinner("Processing resumes..."):
                for resume in resume_files:
                    try:
                        ...
                        profile_insight = analyze_resume_profile(text)
                        ai_summary = generate_ai_summary(text)
                        responsibility_match = match_responsibilities_to_jd(text, jd_text)
                        skill_map, weighted_score, essential_hit, preferred_hit = evaluate_skills_with_experience(text, skill_experience_map)
                        ...
                        results.append({
                            ...
                            "Profile Insight": profile_insight,
                            "AI Summary": ai_summary,
                            "Responsibility Match": responsibility_match
                        })
                    except Exception as e:
            ...
            if results:
                ...
                st.subheader("🤖 AI-Generated Summaries with Recruiter Insight")
                for res in results:
                    st.markdown(f"### {res['Candidate']}")
                    st.markdown("**Profile Understanding:**")
                    st.markdown(res['Profile Insight'])
                    st.markdown("**Responsibility Match Report:**")
                    st.markdown(res['Responsibility Match'])
