# Resume Match and Ranking Script with Authentication (OpenAI SDK v1.0+)
...
def extract_skills_from_jd(jd_text):
    tech_keywords = [
        'Python', 'Java', 'JavaScript', 'React', 'Angular', 'Vue', 'Node.js', 'Ruby on Rails', 'Django',
        'Flask', 'SQL', 'PostgreSQL', 'MongoDB', 'AWS', 'Azure', 'Docker', 'Kubernetes',
        'REST', 'GraphQL', 'CI/CD', 'Git', 'Agile', 'Scrum', 'TDD', 'SaaS', 'HTML', 'CSS',
        'GCP', 'Linux', 'Jenkins', 'Kafka', 'TypeScript', 'Express', 'MySQL'
    ]
    matches = [kw for kw in tech_keywords if re.search(r'\b' + re.escape(kw) + r'\b', jd_text, re.IGNORECASE)]
    return matches[:10], matches[10:20]  # first 5 as essential, next 5 as preferred
    ...
    if st.session_state.authenticated:
        ...
        with st.sidebar:
            ...
            if jd_text_input:
                ...
                essential_input = ", ".join(e_skills)
                preferred_input = ", ".join(p_skills)

            resume_files = st.file_uploader("Resume PDFs", type="pdf", accept_multiple_files=True)

            st.markdown("---")
            essential_input = st.text_area("Essential Skills (auto-filled)", essential_input)
            preferred_input = st.text_area("Preferred Skills (auto-filled)", preferred_input)
