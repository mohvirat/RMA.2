...
def experience_filter(skill, resume_text, required_years):
    # Basic heuristic: look for pattern like "<skill> X years"
    matches = re.findall(rf"{re.escape(skill)}[^\n\r\.;,]*?(\d+(\.\d+)?)\s*(\+\s*)?(yrs|years|YRS|Years)", resume_text, flags=re.IGNORECASE)
    if matches:
        max_years = max(float(m[0]) for m in matches)
...
def extract_skills_from_jd(jd_text):
    ...
    return matches[:10], matches[10:20]
    ...
    if st.session_state.authenticated:
        ...
        with st.sidebar:
            ...
            if jd_text_input:
                ...
                essential_input = e_skills
                preferred_input = p_skills
